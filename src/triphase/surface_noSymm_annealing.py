...
#this code will stop the annealing optimization once a certain error threshold has been achieved
#inputs: mode L p
#mode=trajectory will optimize every trajectory, while anything else will only take into account the average of trajectories
...
Us = list(stim.Tableau.iter_all(2, unsigned=True))
...
if np.shape(QFI)[1] > 0:
    np.savetxt(f'data/structured_qfi/surface/noSymm/L{L}_seed{seed}_qfi_ann.txt', QFI)
