import numpy as np
import sys
from z2_circuits import apply_row_S, apply_row_NS, C_matrices, sim_annealing
...
for pu in np.linspace(0, 1-px, int((1-px)/0.05)+1):
    ...
    for t in range(time):
        if sym:
            state = apply_row_S(state, px, pu, L, invert)
        else:
            state = apply_row_NS(state, px, pu, L, invert)
...
if np.shape(qfi_data)[1] > 0:
    if sym:
        if invert:
            file_name = f'L{L}_seed{seed}_Z2i_'
        else:
            file_name = f'L{L}_seed{seed}_Z2_'
    else:
        if invert:
            file_name = f'L{L}_seed{seed}_Z2NSi_'
        else:
            file_name = f'L{L}_seed{seed}_Z2NS_'
