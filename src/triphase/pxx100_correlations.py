...
if L>65:
    ...
    nps = np.linspace(0, 1, 101)

for pu in nps:
    for pz in nps:
        if np.isclose(pu+pz, 0.9) and L<129:
            ps.append((pz, pu))
        elif pz>0.19 and pz<0.41 and np.isclose(pz+pu, 0.9):
            ps.append((pz, pu))
...
for pz,pu in ps:
    ...
    #np.save(f'/scratch/alirasol/triphase_data/correlations/{file_name}_C_', C)
